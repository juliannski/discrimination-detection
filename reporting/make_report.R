...
options(tinytex.tlmgr.path = tinytex:::find_tlmgr("/Users/avnikothari/.virtualenvs/reticulate-python-env/TinyTex"))
...
p = add_argument(p,
                 "--venv_dir",
                 type = "character",
                 help = "directory for the virtual environment for reticulate")

p = add_argument(p,
                 "--define_conditions",
                 type = "character",
                 help = "Whether to set conditions or find them in the data")

p = add_argument(p,
                 "--condition_params",
                 nargs='+',
                 help = "list od parameters that define conditions")
...
venv_dir = safe.dir(argv$venv_dir);
if(!is.na(argv$condition_params)) {
  print.to.console(argv$condition_params)
  condition_params = unlist(strsplit(argv$condition_params, ","))
} else {
  condition_params = character(0)
}
define_conditions = ifelse(argv$define_conditions == "True", TRUE, FALSE)
...
invisible(file.copy(from = argv$input, to = template_file));
